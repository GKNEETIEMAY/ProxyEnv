...
<p align="center">
  ...
  <a href="https://github.com/GKNEETIEMAY/ProxyEnv/releases/latest"><img alt="正式版本" src="https://img.shields.io/badge/正式版本-v0.1.4-22c55e?style=flat-square"></a>
</p>
...
- 生成经过脱敏的安全诊断报告，并支持简体中文、英文、日语和韩语。
...
> ProxyEnv 负责让 Windows 代理环境变得可见、统一、可控，并帮助用户安全地以代理或直连环境启动应用；不负责长期适配和修改第三方应用内部代理配置。

## 应用网络助手

应用助手帮助用户选择运行中的应用或指定可执行文件，结合当前活动代理、Windows System Proxy、代理环境变量和 TUN / 虚拟网卡状态，对应用当前可能面对的网络环境进行说明。

应用助手不会根据单一网络状态断言应用一定能够联网，而是保持一条保守、可验证的检查流程：

```text
选择应用 → 读取当前网络环境 → 解释当前状态 → 推荐安全操作
         → 使用代理或直连环境启动 → 重新检查结果
```

当存在可用的当前活动代理时，用户可以使用当前代理环境启动新的应用实例、使用直连环境启动新的应用实例，或重新检查当前应用网络环境。选择运行中应用本身只用于确定其可执行文件，不会修改该进程。

ProxyEnv 不修改未知第三方应用内部配置，也不会向已经运行的进程注入或修改环境变量。手动代理引导中的“关闭当前进程并启动新进程”必须由用户明确触发，并会在执行前再次确认未保存内容风险和目标进程身份。

## 多代理客户端与网络观测

**Current — v0.1.4：全局活动代理选择。**

多代理并存时，主页“当前活动代理”会先推荐一个可用客户端，用户可显式切换。环境同步、Mismatch 判断、连通性测试、应用助手和代理启动统一使用当前活动代理。自动刷新不会换选；原代理消失时保留原信息、标记不可用并提示重新选择。切换选择不会自动写环境变量，手动代理应用后也会成为全局目标。此选择在本次运行期间保持，重启 ProxyEnv 后重新推荐。
...
稳定版 v0.1.4 不承担流量转发。尚未发布的 v0.2 远程桥接允许用户明确启用经 ProxyEnv 自有认证 Relay 保护的会话级 SSH 反向转发，但它仍不是通用代理服务器或 VPN；M8 已实现，真实共享 Linux 主机上的不同 UID 验收仍是发布门槛。ProxyEnv 不控制 Clash/v2rayN API、节点、订阅、客户端规则、Windows 系统代理、路由、驱动或系统级环境变量，也不会向运行中进程注入或改写其环境。关闭进程的唯一例外是上述手动引导中经过明确确认和身份校验的重启操作。

本机代理检测、协议探测、TUN 观测、应用枚举与环境变量管理不会静默修改第三方配置。v0.2 开发版仅在用户明确启用远程桥接后，才会修改允许的 Codex / Claude 配置字段，或把本机工具实际可见的 Skill 投影到对应远端工具。配置写入受所有权、备份、校验和恢复保护；Skill 写入使用有界暂存区、SHA-256 清单及所有权校验后的原子启用/移除。Codex `.system` 系统 Skills 不会被投影。ProxyEnv 不会把 Provider 凭据、订阅、节点或认证信息复制到远端工具配置。运行时诊断经过脱敏边界，配置原值按敏感数据处理。外网测试仍须用户主动触发。检查更新访问固定 GitHub 地址；单独点击“下载并安装更新”才获取并验证安装包。详见 [`SECURITY.md`](SECURITY.md)。

## 与环境变量工具对比

下列工具解决的是相邻问题。ProxyEnv 刻意保持更窄的范围，专注代理发现、网络环境解释、代理环境管理，以及安全的新进程启动。

| 工具 | 推荐度 | 适合什么情况 | 主要特点 |
| --- | --- | --- | --- |
| **ProxyEnv** | ⭐⭐⭐⭐⭐ | 代理冲突与应用联网问题 | 统一观察代理 / System Proxy / TUN / 环境变量，支持活动代理选择、安全环境同步与独立应用启动环境 |
