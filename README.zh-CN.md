...
开发进度和发布范围以[路线图](docs/ROADMAP.md)为准。标注为 **Next — v0.1.4** 的功能不包含在 v0.1.3 下载包中。
...
上述文件名中的 `x.x.x` 代表实际发布版本号，例如 `0.1.3`。

ProxyEnv 按项目策略不使用 Windows Authenticode，因此 Windows 可能显示“未知发布者”、Microsoft Defender SmartScreen 或“Windows 已保护你的电脑”。请仅从本仓库下载，并使用 `SHA256SUMS.txt` 与 GitHub Artifact Attestation 验证文件。当前稳定版 v0.1.3 的 NSIS 安装版可以从固定的官方 Release 地址检查、下载并验证 Tauri 签名更新，以被动安装方式替换已登记的旧版本文件，完成后重新启动应用。MSI 与 Portable 包仍从官方 Release 手动更新，避免留下无法自动移除的旧 Portable 文件。
...
检测与定时刷新不会写注册表。当前 v0.1.3 中，检测地址变化可能产生 `Mismatch`，需要用户主动同步。下一版 v0.1.4 中，原端口或进程消失时会保留选择并标记不可用；用户先重新选择可用候选，再明确应用环境变量变更。
