...
Release CI, locked dependencies, the environment transaction, restore conflict detection, CSP, backend IPC authorization, snapshot validation, and Windows Authenticode are P0 requirements for a formal public Windows release. Unsigned development builds and draft artifacts may be used for internal testing only; they must be clearly identified and must not be promoted as a stable public release. If automatic updates are enabled later, signed Tauri Updater artifacts, an embedded public verification key, a fixed HTTPS source, and default anti-downgrade behavior also become P0.

Windows Authenticode establishes publisher identity and improves SmartScreen trust, but it is not the updater integrity mechanism. The formal release pipeline must sign the executable and every Windows installer, apply a trusted timestamp, and verify each resulting signature before publication. ProxyEnv may use Microsoft Artifact Signing or a trusted OV/EV code-signing certificate.
