const extensionEn = {
  ...
  rbModelRemotePreserved: 'The remote Codex model name and capability metadata remain unchanged.', rbModelMappings: 'Advanced model mappings', rbModelNoMappings: 'No fixed mappings.', rbModelTarget: 'CC Switch target model', rbModelAddMapping: 'Add fixed mapping', rbModelObserved: 'Observed remote models:', rbModelRefresh: 'Refresh models',
  rbModelMode: 'Mapping mode', rbModelFixed: 'Fixed local model',
  rbModelAvailableTargets: 'Available local targets', rbModelContext: 'Context:', rbModelReasoning: 'Reasoning:',
  rbModelNoRoute: 'No mapping',
  rbLegacyModelSelectionError: 'A legacy ProxyEnv bridge model was detected, but its original Codex model cannot be proven from the recovery backup. Restore the original model or choose a remote Codex model before enabling again.',
  ...
  rbExtScope: 'Choose the surfaces to enable. Shared client profiles are previewed and written once; separate files keep independent transactions.',
  ...
  rbExtCodexImpact: 'Synchronizes the safe local Codex client profile and ProxyEnv route after creating a recoverable remote backup. Runtime requests remain byte-for-byte semantic passthrough.',
  ...
  rbModelTitle:'Codex model routing',rbModelFollow:'Follow the local Codex model',rbModelFollowHint:'Remote Codex keeps its native model and capability metadata. ProxyEnv resolves only the local request target for each request.',rbModelCurrent:'Current local model',rbModelResolved:'Resolved',rbModelAmbiguous:'Ambiguous',rbModelUnsupported:'Not resolved',rbModelInvalid:'Local profile invalid',rbModelRules:'Compatibility fallbacks',rbModelRulesHint:'Used only when automatic resolution cannot map an incoming model. Automatic local resolution always wins.',rbModelIncoming:'Incoming model',rbModelDisplay:'Local display name',rbModelCanonical:'Canonical model',rbModelAdd:'Add fallback',rbModelRemove:'Remove',rbModelNoRules:'No compatibility fallbacks.',rbModelRuleValid:'Valid',rbModelRuleStale:'Stale',rbModelRuleAmbiguous:'Ambiguous',rbModelRuleInvalid:'Invalid',rbModelSaveError:'Could not save local model routing settings.',
  rbProfileTitle:'Codex client profile',rbProfileFollow:'Follow the local Codex profile',rbProfileHint:'While the Codex bridge is running, ProxyEnv checks file modification times every 2 seconds. A detected change is debounced, fully validated and synchronized by hash.',rbProfileModel:'Selected model',rbProfilePrivacy:'Only the selected model and opaque model catalog are copied. Credentials, provider URLs and unrelated settings are excluded.',rbProfileDisabled:'Sync disabled',rbProfileNotStarted:'Not started',rbProfileSynced:'Synchronized',rbProfileLocalChanged:'Local change detected',rbProfileRemoteChanged:'Remote change detected',rbProfileConflict:'Configuration conflict',rbProfileInvalid:'Local profile invalid',rbProfileUnavailable:'Remote unavailable',rbProfileRestartRequired:'Synchronized · restart Codex',
  rbSkillsTitle:'Remote Skills',rbSkillsHint:'Projects effective local Codex and Claude Skills to the matching remote agent. Changes are checked by metadata and synchronized by hash while the bridge is connected.',rbSkillsEmpty:'No effective local Skills were found.',rbSkillEnable:'Enable',rbSkillDisable:'Disable',rbSkillNotSynced:'Not synchronized',rbSkillSynced:'Synchronized',rbSkillChanged:'Local change detected',rbSkillConflict:'Remote name conflict',rbSkillUnavailable:'Unavailable',rbSkillRestart:'Start a new agent session if the remote tool does not discover the Skill immediately.',rbSkillError:'The Skill could not be projected safely. ProxyEnv did not overwrite foreign remote content.',
};
...
const extensionZh: ExtensionLabels = {
  ...
  rbModelRemotePreserved:'远端 Codex 的模型名称与能力元数据保持不变。',rbModelMappings:'高级模型映射',rbModelNoMappings:'暂无固定映射。',rbModelTarget:'CC Switch 目标模型',rbModelAddMapping:'添加固定映射',rbModelObserved:'已观察到的远端模型：',rbModelRefresh:'刷新模型',
  rbModelMode:'映射方式',rbModelFixed:'固定本地模型',
  rbModelAvailableTargets:'可用本地目标',rbModelContext:'上下文：',rbModelReasoning:'推理等级：',
  rbModelNoRoute:'尚未映射',
  rbLegacyModelSelectionError:'检测到旧版 ProxyEnv 桥接模型，但无法通过恢复备份证明原来的 Codex 模型。请先恢复原模型或选择一个远端 Codex 模型，再重新启用。',
  ...
  rbExtScope:'选择需要启用的入口。共享客户端配置只预览并写入一次；不同文件仍分别执行事务。',
  ...
  rbExtCodexImpact:'创建可恢复的远端备份后，同步安全的本机 Codex 客户端配置与 ProxyEnv 路由；运行时请求保持纯透传。',
  ...
  rbModelTitle:'Codex 模型路由',rbModelFollow:'跟随本机 Codex 模型',rbModelFollowHint:'远端 Codex 保留原生模型名称和能力元数据；ProxyEnv 每次请求只解析本地目标模型。',rbModelCurrent:'当前本机模型',rbModelResolved:'已解析',rbModelAmbiguous:'存在歧义',rbModelUnsupported:'无法解析',rbModelInvalid:'本机配置无效',rbModelRules:'兼容映射',rbModelRulesHint:'仅在自动解析无法匹配传入模型时使用；本机自动解析始终优先。',rbModelIncoming:'传入模型',rbModelDisplay:'本机显示名称',rbModelCanonical:'规范模型',rbModelAdd:'添加映射',rbModelRemove:'移除',rbModelNoRules:'暂无兼容映射。',rbModelRuleValid:'有效',rbModelRuleStale:'已失效',rbModelRuleAmbiguous:'存在歧义',rbModelRuleInvalid:'无效',rbModelSaveError:'无法保存本机模型路由设置。',
  rbProfileTitle:'Codex 客户端配置',rbProfileFollow:'跟随本机 Codex 配置',rbProfileHint:'仅在 Codex Bridge 运行时每 2 秒轻量检查文件修改时间；发现变化后防抖，再完整校验并按哈希同步。',rbProfileModel:'当前模型',rbProfilePrivacy:'只同步选中模型和原样模型目录，不复制凭据、Provider 地址及其它无关设置。',rbProfileDisabled:'同步已关闭',rbProfileNotStarted:'尚未开始',rbProfileSynced:'已同步',rbProfileLocalChanged:'检测到本机变化',rbProfileRemoteChanged:'检测到远端变化',rbProfileConflict:'配置冲突',rbProfileInvalid:'本机配置无效',rbProfileUnavailable:'远端不可用',rbProfileRestartRequired:'已同步 · 请重启 Codex',
  rbSkillsTitle:'远端 Skills',rbSkillsHint:'把本机 Codex 与 Claude 实际可见的 Skill 投影到对应远端工具；桥接期间只轻量检查元数据，变化后按哈希同步。',rbSkillsEmpty:'未发现本机工具实际可见的 Skill。',rbSkillEnable:'启用',rbSkillDisable:'停用',rbSkillNotSynced:'未同步',rbSkillSynced:'已同步',rbSkillChanged:'检测到本机变化',rbSkillConflict:'远端同名冲突',rbSkillUnavailable:'暂不可用',rbSkillRestart:'如果远端工具没有立即发现 Skill，请新建一个 Agent 会话。',rbSkillError:'无法安全投影该 Skill，ProxyEnv 未覆盖远端非托管内容。',
};
const extensionJa: ExtensionLabels = {
  ...
  rbModelRemotePreserved:'リモート Codex のモデル名と機能メタデータは変更しません。',rbModelMappings:'高度なモデルマッピング',rbModelNoMappings:'固定マッピングはありません。',rbModelTarget:'CC Switch 対象モデル',rbModelAddMapping:'固定マッピングを追加',rbModelObserved:'観測済みリモートモデル：',rbModelRefresh:'モデルを更新',
  rbModelMode:'マッピング方式',rbModelFixed:'固定ローカルモデル',
  rbModelAvailableTargets:'利用可能なローカル対象',rbModelContext:'コンテキスト：',rbModelReasoning:'推論：',
  rbModelNoRoute:'マッピングなし',
  rbLegacyModelSelectionError:'旧 ProxyEnv ブリッジモデルを検出しましたが、復旧バックアップから元の Codex モデルを確認できません。元のモデルを復元するか、リモート Codex モデルを選択してから再度有効にしてください。',
  ...
  rbExtScope:'有効にする対象を選択してください。共有クライアント設定は一度だけ確認・書き込みし、別ファイルは個別に処理します。',
  ...
  rbExtCodexImpact:'復元可能なリモートバックアップを作成してから、安全なローカル Codex クライアント設定と ProxyEnv 経路を同期します。実行時要求は純粋に透過転送されます。',
  ...
  rbModelTitle:'Codex モデル経路',rbModelFollow:'ローカル Codex モデルに追従',rbModelFollowHint:'リモート Codex は元のモデル名と機能メタデータを保持し、ProxyEnv は要求ごとにローカル対象モデルだけを解決します。',rbModelCurrent:'現在のローカルモデル',rbModelResolved:'解決済み',rbModelAmbiguous:'曖昧',rbModelUnsupported:'解決不可',rbModelInvalid:'ローカル設定が無効',rbModelRules:'互換フォールバック',rbModelRulesHint:'自動解決で受信モデルを対応付けられない場合のみ使用します。自動解決が常に優先です。',rbModelIncoming:'受信モデル',rbModelDisplay:'ローカル表示名',rbModelCanonical:'正規モデル',rbModelAdd:'フォールバックを追加',rbModelRemove:'削除',rbModelNoRules:'互換フォールバックはありません。',rbModelRuleValid:'有効',rbModelRuleStale:'期限切れ',rbModelRuleAmbiguous:'曖昧',rbModelRuleInvalid:'無効',rbModelSaveError:'ローカルモデル経路設定を保存できません。',
  rbProfileTitle:'Codex クライアント設定',rbProfileFollow:'ローカル Codex 設定に追従',rbProfileHint:'Codex Bridge の実行中だけ、2 秒ごとに更新時刻を軽量確認します。変更時はデバウンス後に全体を検証し、ハッシュで同期します。',rbProfileModel:'選択中のモデル',rbProfilePrivacy:'選択モデルとモデルカタログの原文だけを同期し、認証情報、Provider URL、無関係な設定は除外します。',rbProfileDisabled:'同期無効',rbProfileNotStarted:'未開始',rbProfileSynced:'同期済み',rbProfileLocalChanged:'ローカル変更を検出',rbProfileRemoteChanged:'リモート変更を検出',rbProfileConflict:'設定競合',rbProfileInvalid:'ローカル設定が無効',rbProfileUnavailable:'リモート利用不可',rbProfileRestartRequired:'同期済み・Codex の再起動が必要',
  rbSkillsTitle:'リモート Skills',rbSkillsHint:'ローカルの Codex / Claude から実際に見える Skill を、対応するリモート Agent に投影します。接続中はメタデータだけを確認し、変更時にハッシュで同期します。',rbSkillsEmpty:'有効なローカル Skill が見つかりません。',rbSkillEnable:'有効化',rbSkillDisable:'無効化',rbSkillNotSynced:'未同期',rbSkillSynced:'同期済み',rbSkillChanged:'ローカル変更を検出',rbSkillConflict:'リモート名が競合',rbSkillUnavailable:'利用不可',rbSkillRestart:'リモートツールがすぐに Skill を認識しない場合は、新しい Agent セッションを開始してください。',rbSkillError:'Skill を安全に投影できませんでした。ProxyEnv はリモートの非管理コンテンツを上書きしていません。',
};
const extensionKo: ExtensionLabels = {
  ...
  rbModelRemotePreserved:'원격 Codex 모델 이름과 기능 메타데이터는 변경하지 않습니다.',rbModelMappings:'고급 모델 매핑',rbModelNoMappings:'고정 매핑이 없습니다.',rbModelTarget:'CC Switch 대상 모델',rbModelAddMapping:'고정 매핑 추가',rbModelObserved:'관찰된 원격 모델:',rbModelRefresh:'모델 새로고침',
  rbModelMode:'매핑 방식',rbModelFixed:'고정 로컬 모델',
  rbModelAvailableTargets:'사용 가능한 로컬 대상',rbModelContext:'컨텍스트:',rbModelReasoning:'추론:',
  rbModelNoRoute:'매핑 없음',
  rbLegacyModelSelectionError:'이전 ProxyEnv 브리지 모델을 감지했지만 복구 백업에서 원래 Codex 모델을 확인할 수 없습니다. 원래 모델을 복원하거나 원격 Codex 모델을 선택한 뒤 다시 활성화하세요.',
  ...
  rbExtScope:'활성화할 대상을 선택하세요. 공유 클라이언트 설정은 한 번만 미리 보고 기록하며, 별도 파일은 각각 처리합니다.',
  ...
  rbExtCodexImpact:'복원 가능한 원격 백업을 만든 뒤 안전한 로컬 Codex 클라이언트 설정과 ProxyEnv 경로를 동기화합니다. 런타임 요청은 순수하게 그대로 전달됩니다.',
  ...
  rbModelTitle:'Codex 모델 라우팅',rbModelFollow:'로컬 Codex 모델 따르기',rbModelFollowHint:'원격 Codex는 기존 모델 이름과 기능 메타데이터를 유지하고 ProxyEnv는 요청마다 로컬 대상 모델만 확인합니다.',rbModelCurrent:'현재 로컬 모델',rbModelResolved:'확인됨',rbModelAmbiguous:'모호함',rbModelUnsupported:'확인할 수 없음',rbModelInvalid:'로컬 설정 오류',rbModelRules:'호환 대체 규칙',rbModelRulesHint:'자동 해석이 수신 모델을 연결하지 못할 때만 사용합니다. 자동 해석이 항상 우선합니다.',rbModelIncoming:'수신 모델',rbModelDisplay:'로컬 표시 이름',rbModelCanonical:'표준 모델',rbModelAdd:'대체 규칙 추가',rbModelRemove:'삭제',rbModelNoRules:'호환 대체 규칙이 없습니다.',rbModelRuleValid:'유효',rbModelRuleStale:'오래됨',rbModelRuleAmbiguous:'모호함',rbModelRuleInvalid:'무효',rbModelSaveError:'로컬 모델 라우팅 설정을 저장할 수 없습니다.',
  rbProfileTitle:'Codex 클라이언트 설정',rbProfileFollow:'로컬 Codex 설정 따르기',rbProfileHint:'Codex Bridge가 실행 중일 때만 2초마다 수정 시간을 가볍게 확인합니다. 변경 시 디바운스 후 전체 검증과 해시 동기화를 수행합니다.',rbProfileModel:'선택한 모델',rbProfilePrivacy:'선택 모델과 원본 모델 카탈로그만 동기화하며 자격 증명, Provider URL 및 무관한 설정은 제외합니다.',rbProfileDisabled:'동기화 꺼짐',rbProfileNotStarted:'시작 안 됨',rbProfileSynced:'동기화됨',rbProfileLocalChanged:'로컬 변경 감지',rbProfileRemoteChanged:'원격 변경 감지',rbProfileConflict:'설정 충돌',rbProfileInvalid:'로컬 설정 오류',rbProfileUnavailable:'원격 사용 불가',rbProfileRestartRequired:'동기화됨 · Codex 재시작 필요',
  rbSkillsTitle:'원격 Skills',rbSkillsHint:'로컬 Codex와 Claude에서 실제로 보이는 Skill을 해당 원격 Agent에 투영합니다. 연결 중에는 메타데이터만 확인하고 변경 시 해시로 동기화합니다.',rbSkillsEmpty:'유효한 로컬 Skill을 찾지 못했습니다.',rbSkillEnable:'활성화',rbSkillDisable:'비활성화',rbSkillNotSynced:'동기화 안 됨',rbSkillSynced:'동기화됨',rbSkillChanged:'로컬 변경 감지',rbSkillConflict:'원격 이름 충돌',rbSkillUnavailable:'사용 불가',rbSkillRestart:'원격 도구가 Skill을 즉시 찾지 못하면 새 Agent 세션을 시작하세요.',rbSkillError:'Skill을 안전하게 투영할 수 없습니다. ProxyEnv는 원격의 관리되지 않은 내용을 덮어쓰지 않았습니다.',
};
const flowEn = {
...
ja.rbRemotePortAuto = "通常プロキシは確認できた VS Code ユーザープロキシポート、なければ現在のローカルプロキシポートを優先します。AI ルートは独立して選択します。";
...
ko.rbRemotePortAuto = "일반 프록시는 확인된 VS Code 사용자 프록시 포트, 없으면 현재 로컬 프록시 포트를 우선합니다. AI 라우팅 포트는 별도로 선택합니다.";
...
export function bridgeError(code: unknown, copy: Labels): string {
  ...
  if (["localCodexProfileInvalid","localClaudeProfileInvalid","localCatalogMissing","localCatalogUnsafe"].includes(value)) return copy.rbProfileInvalid;
  if (["remoteProfileConflict","remoteProfileOutOfSync","profileVerifyFailed"].includes(value)) return copy.rbProfileConflict;
  if (["remoteCatalogWriteFailed","remoteConfigWriteFailed"].includes(value)) return copy.rbProfileUnavailable;
  if (value === "codexRestartRequired") return copy.rbProfileRestartRequired;
