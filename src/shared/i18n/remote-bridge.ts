...
const en = {
  ...
  ...foundationEn,
  ...authEn,
  rbCliOverlayMissing: "The remote CLI configuration file has not been created. Use the configuration action above, review the file, and confirm the write before copying a launch command.",
  rbCliOverlayReady: "The remote configuration file was created and verified. Start the CLI with the command below.",
  rbClaudeOnboardingSkipped: "Claude's existing first-run state could not be safely updated. The independent routing file can still be created; Claude Code may show its own setup screen when first launched.",
...
const zh: Labels = {
  ...
  ...foundationZh,
  ...authZh,
  rbCliOverlayMissing:"远端 CLI 配置文件尚未创建。请使用上方配置操作，预览文件并确认写入后再复制启动命令。",
  rbCliOverlayReady:"远端配置文件已创建并完成读回验证，请使用下方命令启动 CLI。",
  rbClaudeOnboardingSkipped:"无法安全更新 Claude 现有的首次引导状态，但仍可创建独立路由配置。首次启动 Claude Code 时可能仍会显示其自身的设置界面。",
...
const ja: Labels = {
  ...
  ...foundationJa,
  ...authJa,
  rbCliOverlayMissing:"リモート CLI 設定ファイルはまだ作成されていません。上の設定操作で内容を確認し、書き込みを承認してから起動コマンドをコピーしてください。",
  rbCliOverlayReady:"リモート設定ファイルを作成し、読み取り検証を完了しました。下のコマンドで CLI を起動してください。",
  rbClaudeOnboardingSkipped:"Claude の既存の初回起動状態を安全に更新できませんでした。独立したルーティング設定は作成できますが、初回起動時に Claude Code の設定画面が表示される場合があります。",
...
const ko: Labels = {
  ...
  ...foundationKo,
  ...authKo,
  rbCliOverlayMissing:"원격 CLI 설정 파일이 아직 생성되지 않았습니다. 위의 설정 작업에서 파일을 검토하고 쓰기를 확인한 뒤 실행 명령을 복사하세요.",
  rbCliOverlayReady:"원격 설정 파일을 생성하고 읽기 검증을 완료했습니다. 아래 명령으로 CLI를 시작하세요.",
  rbClaudeOnboardingSkipped:"Claude의 기존 최초 실행 상태를 안전하게 업데이트할 수 없습니다. 독립 라우팅 설정은 계속 만들 수 있지만 처음 실행할 때 Claude Code 설정 화면이 표시될 수 있습니다.",
