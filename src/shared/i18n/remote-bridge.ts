...
const flowEn = {
  ...
  rbLaunchProxyTerminal: "Launch proxy terminal", rbTerminalLaunched: "PowerShell proxy terminal launched", rbTerminalLaunchHint: "Open a new PowerShell window, connect to this remote environment with OpenSSH, and inject the current bridge proxy automatically.", rbTerminalAuthHint: "For a plain password login, ProxyEnv reuses the password only during this bridge session. It is protected with Windows user encryption, never written to disk, and cleared when you switch targets, disconnect, or exit.", rbAdvancedCopy: "Advanced: use in an existing terminal", rbManagedShellScope: "After these variables are applied, they affect only that shell and its child processes. ProxyEnv does not modify .bashrc, .profile, /etc/environment, or other startup files.",
