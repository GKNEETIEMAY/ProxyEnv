# VS Code AI 扩展桥接实现可行性审计（历史归档）

审计日期：2026-09-06。代码基线：`v0.2.0dev` / `338c5fc`。

> 当前实现说明：下文是当时的可行性审计，不代表现行开关行为。开发分支后来改为 Claude CLI 与 VS Code Remote Extension 共用远端 `~/.claude/settings.json` 的模型配置投影与单一事务，不再由当前开关修改 `claudeCode.environmentVariables`。本审计指出的扩展独立登录检查风险仍在：共享配置写入成功不等于扩展已能登录或发出真实模型请求，发布前必须实机验证。
