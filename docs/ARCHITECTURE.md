...
The status surface separates transport health from product completion. It reports the selected target, SSH process, local endpoints, and remote reverse forwards, then provides capability-specific next steps. Proxy bridging primarily launches a visible Windows PowerShell host, which runs the system OpenSSH client with backend-generated `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` state scoped to the remote shell and its descendants. The GUI-subsystem backend creates that host with `CreateProcessW`, a new console, and an untouched `STARTUPINFO` so invalid parent standard handles cannot make the terminal non-interactive. OpenSSH arguments cross the PowerShell boundary as a JSON environment payload and are splatted as data rather than interpolated into a command string. It does not edit `.bashrc`, `.profile`, `/etc/environment`, or require sudo. For eligible plain-password sessions, post-connect bridge operations receive only a DPAPI-protected payload and obtain the temporary plaintext through ProxyEnv's restricted `SSH_ASKPASS` mode; key passphrases, OTP and unknown prompts are never reused. Explicit exports remain behind an advanced entry for an already authenticated terminal. Connectivity testing remains opt-in. The target and endpoint come from the connected backend summary, SSH configuration is fingerprint-checked again, and no frontend-provided shell command is accepted. CC Switch AI routing is independent from the general proxy capability, but remote AI access still requires its SSH reverse forward to remain connected. After reconnect, backend `status` operations inspect the remote ownership marker and semantic route port, then rehydrate configured tool state without requiring another write. Claude CLI apply merges only routing/authentication takeover fields into `~/.claude/settings.json`; it does not modify `~/.claude.json`, onboarding state or project trust. A successful tunnel or configuration write does not imply that a CLI, extension, provider route, or model request has been verified. Disconnect closes reverse forwards but deliberately keeps enabled remote access; disabling access is a separate confirmed restore transaction.

### Remote tool adapters / 远程工具适配器

M5 moves the first CLI integrations behind matching Rust and TypeScript `RemoteToolAdapter` registries. Codex CLI and Claude Code CLI each own their identity, compatibility gate, configuration path and managed-field renderer, preview/enable/disable transaction entry points, launch command, supported route modes, and verification state. The implementation retains `apply`/`restore` operation names internally for compatibility. Both tools use their official shared user settings so plain `codex` and `claude` receive the route; their UI exposes one enable/disable switch per CLI while the existing preview and confirmation transaction remains mandatory. The Codex adapter explicitly sets `requires_openai_auth = false`: the local CC Switch route owns upstream authentication, while remote `auth.json` remains untouched. The status parser recognizes the erroneous earlier development value `true` only as a legacy ProxyEnv-owned shape and requires a reviewed re-enable transaction to migrate it. `RemoteBridgePage.vue` and `RemoteToolDialog.vue` iterate the registry instead of branching on tool names; unknown tool IDs are rejected by the backend before any SSH operation.

M5 首批迁移 Codex CLI 与 Claude Code CLI。两端适配器统一负责工具标识、版本兼容性、配置路径与托管字段、预览/启用/停用、启动命令、支持的路由模式和验证状态；内部继续使用兼容的 `apply` / `restore` 操作名。两者都使用官方共享用户配置，启用后直接执行 `codex` / `claude`；界面每个 CLI 只保留一个启用/停用开关，仍必须经过预览和确认事务。页面与弹窗只消费注册表，不再用工具名称堆叠条件分支。配置写入后状态仅为 `VerifyPending`，不会把“文件已写入”冒充“真实模型请求已验证”。

Codex model selection is deliberately local-only. The remote transaction owns `model_provider`, a neutral managed `model = "proxyenv-bridge"`, the `model_catalog_json` selector, and the `proxyenv_bridge` provider block. It creates a fixed ProxyEnv-owned capability profile solely to describe standard remote streaming/tool behavior; this profile never contains or mirrors the selected local upstream model. Exact original model/provider/catalog-selector values are restored and the owned profile is removed on disable; a user change to a managed field or a pre-existing reserved profile path fails closed as `configConflict`. A session-owned loopback relay sits between the SSH reverse forward and the fixed local CC Switch endpoint. It parses only bounded Responses API JSON requests, rereads local `~/.codex/config.toml` for every request, and rewrites only the request `model` before streaming the upstream response without semantic transformation. Exact local catalog slug/display matches remain preferred. When a safe single-entry routing catalog describes the selected route but differs from the configured request alias, the configured `model` remains canonical; this matches CC Switch's atomic route projection without coupling ProxyEnv to its database. Multi-entry ambiguity, invalid, missing, linked, escaping or oversized local inputs still fail closed. Automatic resolution has priority over optional local compatibility rules. Claude and non-Responses traffic use passthrough behavior. Local routing settings are stored outside the WebView and exposed through allowlisted Tauri DTOs.
